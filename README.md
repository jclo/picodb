...
PicoDB manages the documents that are similar to a MongoDB's collection. It provides a flexible API to insert, update, delete and find documents.
...
addOneTimeEventListener | Adds an event listener that fires once.
removeEventListener | Removes the event listener.
on | Alias of addEventListener.
one | Alias of addOneTimeEventListener.
off | Alias of removeEventListener.
